...
$config->setHost("https://{$server}/v3");

if (strtolower($authType) === 'apikey') {
    $config->setUsername($authKey);
} elseif (strtolower($authType) === 'oauth') {
    $config->setAccessToken($authKey);
}

$apiRequester = classFromOperationId(
    $jsonData['operationId'],
    $config
);
...
function classFromOperationId(
    string $operationId,
    HelloSignSDK\Configuration $config
): ApiRequesterI {
    switch ($operationId) {
        case 'accountCreate':
